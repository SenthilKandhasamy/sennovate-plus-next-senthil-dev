...
function renderPoint2(point: any) {
  return {
    key: point.title,
    offering: point.title,
    on-prem_essential: <div className="text-center px-2">{point.on-prem_essential}</div>,
    on-prem_advance: <div className="text-center px-2">{point.on-prem_advance}</div>,
...
export default function ServiceOfferingTable2({ offering }: Props) {
  const columns = [
    {
      key: "offering",
      label: offering.heading,
  ...
  const rows = (offering.points_2 || []).reduce<any[]>((a, c) => {
